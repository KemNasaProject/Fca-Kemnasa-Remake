...
Lưu Ý! Đây Là Sản Phẩm Được Kemnasa Remake ( Chính Bởi Facebook-Chat-Api Của Schmavery, Tác Giả Không Chịu Trách Nghiệm Nào ! ), Nếu Có Lỗi Hãy Thử Sử Dụng Sang Sản Phẩm Khác !
...
```bash
npm i fca-kemnasa-remake
```
or
```bash
npm install fca-kemnasa-remake
```
...
```bash
npm install fca-kemnasa-remake@latest
```
Hoặc
```bash
npm i fca-kemnasa-remake@latest
```
...
```javascript
const login = require("fca-kemnasa-remake"); // lấy từ lib ra 
...
```js
    var login = require('fca-kemnasa-remake')
```
...
```js
const fs = require("fs");
const login = require("fca-kemnasa-remake");
